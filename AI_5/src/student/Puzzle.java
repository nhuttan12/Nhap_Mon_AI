...
public class Puzzle {
	...
	public int computeH1(Node currentState) {
		...
		for (int i=0; i< MAX_ROW; i++){
			for (int j = 0; j < MAX_COL; j++) {
				int tileCount=currentState.getTile(i,j);
				int tileGoal=goalState.getTile(i,j);
				if(tileCount!=0 && tileCount!=tileGoal){
					output++;
				}
			}
		}
	...
	public int computeH2(Node currentState) {
		int result = 0;
		/* Enter your code here */
		for (int tile=0; tile<MAX_ROW; tile++){
		}
		return result;
	}
	...
	public Node moveWhiteTile(Node currentState, char operator) {
		...
		if (operator == 'u') {// Case-1: Move tile UP
			...
			int col=whiteTile[1];
			if(row>=0){
				int temp=currentState.getTile(row,col);
				result.updateTile(row,col,0);
				result.updateTile(whiteTile[0],whiteTile[1],temp);
